...
const Register = () => {
  const [formData, setFormData] = useState({
    name: '', email: '', voterId: '', fingerprintId: '', password: '', confirmPassword: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(res.data.user.role === 'admin' ? '/admin' : '/voter');
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-600 via-primary-700 to-purple-800 p-4">
      <div className="w-full max-w-2xl animate-scale-in">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-primary-200 text-sm mt-1">Register to participate in secure elections</p>
        </div>

        {/* Card */}
        <div className="bg-white dark:bg-surface-900 rounded-2xl shadow-2xl p-8 border border-white/20 dark:border-surface-800">
          <h2 className="text-xl font-bold text-surface-900 dark:text-white mb-1">Voter Registration</h2>
          <p className="text-sm text-surface-500 dark:text-surface-400 mb-6">Fill in your details to create your voting account</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" name="voterId" value={formData.voterId} onChange={handleChange} placeholder="Enter unique Voter ID" required className={inputClasses} />
              </div>
              <div>
            ...
            >
              {loading ? (<><span className="btn-spinner" /> Registering...</>) : 'Create Account'}
            </button>
          </form>
