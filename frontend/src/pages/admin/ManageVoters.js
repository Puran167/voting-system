...
const ManageVoters = () => {
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', voterId: '', fingerprintId: '', password: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ name: '', email: '', voterId: '', fingerprintId: '', password: '' });
      setShowForm(false);
      fetchVoters();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        >
          {showForm ? t('admin.cancel') : t('admin.addVoter')}
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-200 dark:border-surface-800 p-6 animate-slide-in">
          <h2 className="text-lg font-bold text-surface-900 dark:text-white mb-4">{t('admin.addNewVoter')}</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-surface-700 dark:text-surface-300 mb-1.5">{t('admin.email')}</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" name="voterId" value={formData.voterId} onChange={handleChange} required className={inputClasses} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-surface-700 dark:text-surface-300 mb-1.5">{t('admin.fingerprintId')}</label>
                <input type="text" name="fingerprintId" value={formData.fingerprintId} onChange={handleChange} required className={inputClasses} />
              </div>
            </div>
            <div>
              <label className="block text-sm font-semibold text-surface-700 dark:text-surface-300 mb-1.5">{t('admin.password')}</label>
            ...
            <button type="submit" disabled={loading} className="px-6 py-2.5 rounded-xl bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold shadow-lg shadow-primary-500/30 disabled:opacity-50 transition-colors flex items-center gap-2">
              {loading ? (<><span className="btn-spinner" /> {t('admin.adding')}</>) : t('admin.addVoter')}
            </button>
          </form>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-200 dark:border-surface-800">
                ...
                <th className="text-left px-6 py-4 font-semibold text-surface-500 dark:text-surface-400 uppercase text-xs tracking-wider">{t('admin.fingerprintLabel')}</th>
            ...
            <tbody className="divide-y divide-surface-100 dark:divide-surface-800">
              {voters.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-6 py-12 text-center text-surface-400 dark:text-surface-500">
                ...
                voters.map((voter) => (
                  <tr key={voter._id} className="hover:bg-surface-50 dark:hover:bg-surface-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-surface-600 dark:text-surface-400 font-mono text-xs">{voter.fingerprintId}</td>
